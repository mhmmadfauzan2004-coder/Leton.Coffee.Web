...
BEGIN
  ...
  WHERE LOWER(TRIM(nama_lengkap)) = LOWER(v_clean_nama)
     OR (LENGTH(v_digits) >= 9 AND (nomor_hp = v_digits OR nomor_hp = '0' || SUBSTRING(v_digits FROM 3) OR nomor_hp = v_clean_nama))
  LIMIT 1;
...
-- B.2 Login Customer By Phone (Nomor HP & Password)
